...
export async function POST(req: NextRequest) {
  try {
    ...
    if (currentUser?.avatar) {
      // Extrair path do avatar antigo (formato: https://...supabase.co/storage/v1/object/public/images/avatars/...)
    ...
    if (error || !url) {
      console.error('[admin][avatar] Erro no upload Supabase:', error);
      return NextResponse.json(
    ...
    console.error('[admin][avatar] Erro ao fazer upload:', error);
    return NextResponse.json(
