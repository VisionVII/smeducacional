...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Iniciando login de administrador com:', formData.email);
      ...
      if (result.ok) {
        console.log(
          '✅ Login de administrador bem-sucedido! Aguardando cookie...'
        );
        toast({
        ...
        try {
          ...
          if (session?.user?.role === 'ADMIN') {
            ...
            toast({
              title: 'Acesso negado',
              description:
                'Esta conta não tem permissão de administrador. Use o login correto.',
              variant: 'destructive',
            });
            setIsLoading(false);
          }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-red-50 to-white dark:from-gray-900 dark:to-gray-800 p-4">
      <Card className="w-full max-w-sm relative shadow-2xl backdrop-blur-2xl bg-gradient-to-br from-background/95 to-background/90 border border-primary/30 hover:border-primary/50 transition-all duration-500">
