...
**Status:** ⚠️ Em validação (ajuste de versão do jsPDF por disponibilidade no npm)
...
- **Ação:** ⚠️ Ajustado em `package.json` (versão existente no npm). Reavaliar mitigação.
...
- **jsPDF (BMP DoS / PDF Injection / XMP / Race Condition):** Requer revalidação da mitigação após ajuste de versão
